...
def loadModel(path):
    model = VariationalAutoencoder(latent_features, TEACHER_FORCING,
                                   learning_rate)
...
def showReconstructionsMultiNotes(multi_notes: torch.Tensor, x: torch.Tensor):
    ...
    for i, seq in enumerate(multi_notes_np):
        ...
        midi = builder.midi_from_piano_roll(
            midi_out[:, :-1])  # Select all notes but the silent one
...
def showReconstructions(model, x_hat, x):
    ...
    for i, seq in enumerate(x_hat_np):
        ...
        np.savetxt("midi/csv_midi_out_{}.csv".format(i),
                   midi_out,
                   delimiter=";")
...
def generateFromLatentSpace(model, gen_batch=10, showPlot=True):
    ...
    for i in range(totalbars):
        decoder_hidden_gen = (torch.randn(1,
                                          gen_batch,
                              ...
                              torch.randn(1,
                                          ...
                                          decoders_initial_size,
                                          device=device))
        embedding_gen, conductor_hidden_gen = model.conductor(
            z_gen[:, i, :].view(gen_batch, 1, -1), conductor_hidden_gen)
        ...
        for _ in range(sequence_length):
            ...
            note_gen, decoder_hidden_gen = model.decoder(
                e_gen, decoder_hidden_gen)
    ...
    if not showPlot:
        ...
        for i, seq in enumerate(notes_np):
            ...
            np.savetxt("midi/gen_csv_midi_out_{}.csv".format(i),
                       midi_out,
                       delimiter=";")
...
if __name__ == "__main__":
    ...
    midi_dataset = MidiDataset(csv_file='piano_rolls_a_small_fraction.csv',
                               transform=transform)
    ...
    model.set_scheduled_sampling(
        1.)  # Please use teacher forcing for validations
    outputs = model(x)
